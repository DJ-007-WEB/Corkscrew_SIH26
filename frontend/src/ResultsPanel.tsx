      ...
      {entries.map(([state, probability]) => (
        <div key={state} className="grid grid-cols-[52px_1fr_52px] items-center gap-3">
          ...
          <div className="h-2 rounded-full bg-[var(--bp-border)] overflow-hidden">
            <div className="h-full rounded-full transition-all" style={{ width: `${(probability / max) * 100}%`, background: "var(--bp-cyan)", boxShadow: "0 0 10px var(--bp-cyan-dim)" }} />
          </div>
          <span className="font-mono text-xs text-right text-[var(--bp-text-dim)]">{(probability * 100).toFixed(1)}%</span>
        </div>
      ))}
    </div>
  );
      ...
      <div className="border border-[var(--bp-border)] rounded-md p-4">
        ...
        <div className="grid lg:grid-cols-[180px_1fr] gap-4">
          ...
          <div className="border border-[var(--bp-border)] rounded-md p-3">
            ...
            <div className="space-y-2">
              {currentActive.map(([state, probability]) => (
                <div key={state} className="grid grid-cols-[52px_1fr_52px] items-center gap-3">
                  <span className="font-mono text-xs text-[var(--bp-text)]">|{state}⟩</span>
                  <div className="h-2 rounded-full bg-[var(--bp-border)] overflow-hidden">

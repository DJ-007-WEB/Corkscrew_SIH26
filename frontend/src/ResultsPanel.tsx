...
function ProbabilityBars({ probabilities, compact = false }: { probabilities: Record<string, number>; compact?: boolean }) {
  ...
  if (entries.length === 0) return <p className="text-xs font-mono text-[var(--bp-text-faint)]">No measurable outcomes.</p>;
  return (
...
export default function ResultsPanel({ result, onStepChange }: ResultsPanelProps) {
  ...
  const evolutionDescription = useMemo(() => gateExplanation(currentGate, currentProbabilities), [currentGate, currentProbabilities]);

  return (
    <section className="bp-panel p-4 sm:p-5 space-y-4">
      ...
      {/* Live simulation: intentionally compact and directly above the static results. */}
      <div className="border border-[var(--bp-border)] rounded-md p-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
          <div>
            <p className="text-xs font-mono uppercase tracking-wider text-[var(--bp-text-dim)]">Live simulation</p>
            <p className="text-[11px] text-[var(--bp-text-faint)] mt-1">Synced with the circuit step-by-step. Selecting a step highlights its gate above.</p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => selectStep(selectedStep - 1)} disabled={selectedStep <= 0} className="w-8 h-8 rounded border border-[var(--bp-border-strong)] text-xs font-mono text-[var(--bp-text-dim)] hover:border-[var(--bp-cyan)] hover:text-[var(--bp-cyan)] disabled:opacity-30">‹</button>
            <button onClick={() => setPlaying((value) => !value)} disabled={stepCount <= 1} className="min-w-24 px-3 py-1.5 rounded border border-[var(--bp-border-strong)] text-[10px] font-mono text-[var(--bp-cyan)] hover:border-[var(--bp-cyan)] disabled:opacity-40">{playing ? "Ⅱ Pause" : "▶ Play"}</button>
            <button onClick={() => selectStep(selectedStep + 1)} disabled={selectedStep >= stepCount - 1} className="w-8 h-8 rounded border border-[var(--bp-border-strong)] text-xs font-mono text-[var(--bp-text-dim)] hover:border-[var(--bp-cyan)] hover:text-[var(--bp-cyan)] disabled:opacity-30">›</button>
          </div>
        </div>

        <div className="flex flex-wrap gap-2 mb-3">
          {result.steps.map((step) => (
            <button key={step.step} onClick={() => { setPlaying(false); selectStep(step.step); }} className={`px-2.5 py-1 rounded border text-[10px] font-mono transition-colors ${selectedStep === step.step ? "border-[var(--bp-cyan)] text-[var(--bp-cyan)] bg-[var(--bp-cyan)]/5" : "border-[var(--bp-border)] text-[var(--bp-text-faint)] hover:text-[var(--bp-text-dim)]"}`}>
              {step.step === 0 ? "START" : `GATE ${step.gate_index !== null && step.gate_index !== undefined ? step.gate_index + 1 : step.step}`}
            </button>
          ))}
        </div>

        <div className="grid lg:grid-cols-[minmax(180px,0.45fr)_minmax(0,1fr)] gap-3">
          <div className="border border-[var(--bp-border)] rounded-md px-3 py-2.5">
            <div className="flex items-center justify-between gap-2">
              <p className="text-[10px] font-mono uppercase tracking-wider text-[var(--bp-text-faint)]">Current operation</p>
              <span className="text-[10px] font-mono text-[var(--bp-cyan)]">STEP {selectedStep}</span>
            </div>
            <p className="text-sm font-mono text-[var(--bp-text)] mt-1">{gateLabel(currentGate)}</p>
            <p className="text-[10px] text-[var(--bp-text-faint)] mt-2 leading-relaxed">{evolutionDescription}</p>
          </div>
          <div className="border border-[var(--bp-border)] rounded-md px-3 py-2.5">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] font-mono uppercase tracking-wider text-[var(--bp-text-faint)]">State at step {selectedStep}</p>
      ...
      {/* Static results: final probabilities and amplitudes stay together below the live simulation. */}
      <div className="grid lg:grid-cols-2 gap-4">
        <div className="border border-[var(--bp-border)] rounded-md p-4">
          ...
          <details className="mt-4">
            ...
            <div className="mt-3 max-h-36 overflow-y-auto bp-scrollbar border border-[var(--bp-border)] rounded-sm">
        ...
        <div className="border border-[var(--bp-border)] rounded-md p-4">
          ...
          <div className="max-h-36 overflow-y-auto bp-scrollbar space-y-1">
